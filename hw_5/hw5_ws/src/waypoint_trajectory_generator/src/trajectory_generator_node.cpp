...
// Param from launch file
    ...
    OsqpEigen::Solver slover;   // 这玩意只要初始化后就需要全生命周期存在，不然就会报错
...
{ 
    ...
    */
    ...
    double dis_acc = 0.5*_Vel*pow(t_acc,2);

    for(int k=0; k<Path.rows()-1; k++){
        double dist = (Path.row(k+1)-Path.row(k)).norm();
        if(dist < dis_acc*2){
            time(k) = sqrt(dist/_Acc);
        }
        else{
            time(k) = t_acc*2 + (dist-dis_acc*2)/_Vel;
        }
    }
