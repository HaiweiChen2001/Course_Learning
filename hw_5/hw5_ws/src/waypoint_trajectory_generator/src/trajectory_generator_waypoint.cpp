...
// 在线性约束矩阵的指定位置插入系数
void TrajectoryGeneratorWaypoint::InsertCoff(
    const int row, 
    const int col, 
...
// 获取等式约束矩阵，也就是矩阵Aeq
void TrajectoryGeneratorWaypoint::GetLinearConstraintsMatrix(
                const int n_seg,
                const int d_order,
                const Eigen::VectorXd &Time,
                Eigen::SparseMatrix<double> & linearMatrix)
            ...
            OsqpEigen::Solver &solver)          
{
    ...
    if(!solver.data()->setHessianMatrix(hession)){
        cout << "设置hession矩阵失败";
